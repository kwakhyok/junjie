/*
 Basic Primitives orgDiagram v1.0.33
...
primitives.common.Graphics.prototype.resize=function(a,b,c){a=this.m_placeholders[a];null!=a&&this.resizePlaceholder(a,b,c)};primitives.common.Graphics.prototype.setTransform=function(a,b,c,d){a=this.m_placeholders[a];null!=a&&(a.invertArea=b,a.invertHorizontally=c,a.invertVertically=d)};
...
primitives.orgdiagram.Controller.prototype._refresh=function(){this._updateLayout();this.m_scrollPanel.css({display:"none","-webkit-overflow-scrolling":"auto"});this._updateScale();this._setItemsIntervals();this._readTreeItems();this._positionTreeItems();this.graphics.resize("placeholder",this.m_placeholderRect.width,this.m_placeholderRect.height);this.graphics.begin();this._redrawTreeItems();this._redrawConnectors();this._drawHighlight();this._hideHighlightAnnotation();this._drawCursor();this.graphics.end();
this.m_scrollPanel.css({display:"block"});this._centerOnCursor();this.m_scrollPanel.css({"-webkit-overflow-scrolling":"touch"})};primitives.orgdiagram.Controller.prototype._setItemsIntervals=function(){this._itemsInterval[1]=this.options.normalItemsInterval;this._itemsInterval[2]=this.options.dotItemsInterval;this._itemsInterval[3]=this.options.lineItemsInterval;this._itemsInterval[4]=this.options.lineItemsInterval};
...
primitives.orgdiagram.Controller.prototype._centerOnCursor=function(){var a;null!==this._cursorTreeItem&&(a=this.graphics.activate("placeholder",5),a.transformPoint(this._cursorTreeItem.actualPosition.horizontalCenter()*this.scale,this._cursorTreeItem.actualPosition.verticalCenter()*this.scale,this,function(a,c){this.m_scrollPanel.scrollLeft(a-this.m_scrollPanelRect.horizontalCenter());this.m_scrollPanel.scrollTop(c-this.m_scrollPanelRect.verticalCenter())}))};
...
primitives.orgdiagram.Controller.prototype._getTreeItemForMousePosition=function(a,b){var c=null,d,e,f,g,h,j,m,k,l,i=this.graphics.activate("placeholder",5),a=a/this.scale,b=b/this.scale;i.transformPointBack(a,b,this,function(a,b){d=0;for(e=this._treeLevels.length;d<e;d+=1)if(g=this._treeLevels[d],b>g.topConnectorShift&&b<=g.connectorShift){j=h=null;k=0;for(f=g.treeItems.length;k<f;k+=1)switch(m=this._treeItems[g.treeItems[k]],m.actualVisibility){case 1:if(m.actualPosition.contains(a,b)){c=m;return}case 2:case 3:if(l=
Math.abs(m.actualPosition.horizontalCenter()-a),null===j||l<j)j=l,h=m}c=h;break}});return c};
primitives.orgdiagram.Controller.prototype._drawHighlightAnnotation=function(){var a=primitives.common,b,c,d,e,f,g,h,j;if(null!==this._highlightTreeItem)switch(this._highlightTreeItem.actualVisibility){case 2:case 3:case 1:c=this._highlightTreeItem;d=new a.Rect(this.m_scrollPanel.scrollLeft(),this.m_scrollPanel.scrollTop(),Math.min(this.m_scrollPanelRect.width-25,this.m_placeholderRect.width),Math.min(this.m_scrollPanelRect.height-25,this.m_placeholderRect.height));b=this.graphics.activate("placeholder",
5);b.transformPoint(c.actualPosition.horizontalCenter(),c.actualPosition.verticalCenter(),this,function(m,k){f=new a.Point(m,k);1!=this._highlightTreeItem.actualVisibility||!d.contains(m,k)?(g=this._getAnnotationPosition(f,d,c.template.itemSize),e=new a.Rect(g),e.addRect(f.x,f.y),e.offset(50),j=e.getCSS(),j.display="inherit",j.visibility="inherit",this.m_calloutPlaceholder.css(j),f.x-=e.x,f.y-=e.y,g.x-=e.x,g.y-=e.y,h=new a.RenderEventArgs,h.context=c.itemConfig,h.isCursor=c.isCursor,h.isSelected=
c.isSelected,h.templateName=c.template.name,this.graphics.resize("calloutplaceholder",e.width,e.height),b=this.graphics.activate("calloutplaceholder",6),this.graphics.template(g.x,g.y,g.width,g.height,0,0,g.width,g.height,c.template.itemTemplate,c.template.itemTemplateHashCode,c.template.itemTemplateRenderName,h,null),this.pointerPlacement=0,this.m_calloutShape.cornerRadius=this.options.calloutCornerRadius,this.m_calloutShape.offset=this.options.calloutOffset,this.m_calloutShape.opacity=this.options.calloutOpacity,
this.m_calloutShape.lineWidth=this.options.calloutLineWidth,this.m_calloutShape.pointerWidth=this.options.calloutPointerWidth,this.m_calloutShape.borderColor=this.options.calloutBorderColor,this.m_calloutShape.fillColor=this.options.calloutfillColor,this.m_calloutShape.draw(f,g)):this.m_calloutPlaceholder.css({display:"none",visibility:"hidden"})});break;case 4:this.m_calloutPlaceholder.css({display:"none",visibility:"hidden"})}};
...
(this._treeItems[d.visualChildren[0]].relationDegree=1);break;case 3:if(3<d.visualChildren.length){a=d.visualChildren.slice(0);d.visualChildren.length=0;j=Math.min(this.options.maximumColumnsInMatrix,Math.ceil(Math.sqrt(a.length)));m=Math.ceil(a.length/j);k=Math.ceil(j/2);for(l=0;l<k;l+=1){e=d;for(i=0;i<m;i+=1){f=this._getMatrixItem(a,2*l,i,j,m);g=this._getMatrixItem(a,2*l+1,i,j,m);void 0===b[i]&&(b[i]=[],c[i]=[]);null!==f&&(0==l&&(f.relationDegree=1),e.visualChildren.push(f),f.visualParentId=e.id,
f.connectorPlacement=3,c[i].push(f));if(null!==f||null!==g)h=this._getNewTreeItem({visibility:4,visualParentId:e.id}),e.visualChildren.push(h),e.visualAggregatorId=h.id,b[i].push(h);null!==g&&(e.visualChildren.push(g),g.visualParentId=e.id,g.connectorPlacement=7,c[i].push(g));e=h}}2<j&&(d.visualAggregatorId=null)}break;case 1:a=d.visualChildren.slice(0);g=d.visualChildren.length=0;for(j=a.length;g<j;g+=1){f=a[g];h=this._getNewTreeItem({visibility:4,visualParentId:d.id});d.visualAggregatorId=h.id;
